...
export const Select = ({
  ...
  const pattern = required?.pattern?{pattern: required.pattern}:{}
  ...
  const selectAction = (e) => {
    ...
    action({...pattern, name: name, value: resp, type: 'select' });
  };
  ...
  const selectAll = (e) => {
    action({...pattern, name: name, value: e ? options : [], type: 'select' });
  };
...
export const FilterSelect = ({
  ...
  const onActionFilter = (e) => {
    setFilterState(e);
    // if (e.length >= 4) {
      action({value: e, ...pattern});
    // }
  };
  return (
    <div className="select-filter">
      <div className="input-actions">
        <Button cy={`SelectFilter${cy}`}onClick={() => action({value: filterState})} title={title}>
          {filter}
        </Button>
      </div>
      <input
      ...
      <div className="input-actions">
        <Button
          ...
          onClick={() => [cleanFilter(), action({value: ''})]}
          cy={`CleanFilter${cy}`}
        >
          {clean}
        </Button>
